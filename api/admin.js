...
import { sendNewsletterForArticles } from './_lib/mailer.js';
...
export default async function handler(req, res) {
  try {
    ...
    if (body.action === 'save') {
      ...
      // Detectar qué notas son realmente nuevas (no estaban antes) para avisar por mail.
      let newArticles = [];
      try {
        const before = await readData();
        const beforeIds = new Set((before.articles || []).map(a => a.id));
        newArticles = articles.filter(a => !beforeIds.has(a.id));
      } catch (e) {
        console.error('[admin/save] no se pudo comparar contra el contenido anterior:', e);
      }

      await saveData({ articles, fixtures });

      let newsletter = { sent: 0 };
      if (newArticles.length) {
        try { newsletter = await sendNewsletterForArticles(newArticles, req); }
        catch (e) { console.error('[admin/save] falló el envío del newsletter:', e); }
      }

      return res.status(200).json({ ok: true, articles: articles.length, fixtures: fixtures.length, newsletterSent: newsletter.sent || 0 });
    }
