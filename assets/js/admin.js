(() => {
  ...
  async function saveData() {
    const result = await api('save', { articles: state.articles, fixtures: state.fixtures });
  ...
  async function saveArticle(e){
    ...
    try {
      ...
      closeModals(); renderAll(); switchSection('articles');
      const msg = result?.newsletterSent ? `Noticia publicada. Newsletter enviado a ${result.newsletterSent} suscriptor(es).` : 'Noticia publicada correctamente.';
      toast(msg);
